...
exports.seedDefaultTokens = exports.deleteToken = exports.updateToken = exports.addToken = exports.getTokens = void 0;
...
const getTokens = async (req, res) => {
    try {
        ...
        const tokens = await Token_1.default.find(filter).sort({ createdAt: 1 });
        res.json({
            success: true,
            data: { tokens },
        });
...
const addToken = async (req, res) => {
    try {
        ...
        const existingToken = await Token_1.default.findOne({ address: address.toLowerCase() });
        if (existingToken) {
        ...
        await token.save();
        // Create audit log
        await (0, auditLog_1.createAuditLog)('token_added', addedBy, req, {
...
const updateToken = async (req, res) => {
    try {
        ...
        const token = await Token_1.default.findOne({ address: address.toLowerCase() });
        if (!token) {
        ...
        await token.save();
        // Create audit log
        await (0, auditLog_1.createAuditLog)('token_updated', updatedBy, req, {
            tokenAddress: address,
...
const deleteToken = async (req, res) => {
    try {
        ...
        const token = await Token_1.default.findOneAndDelete({ address: address.toLowerCase() });
        if (!token) {
            return res.status(404).json({
                success: false,
                error: { message: 'Token not found' },
        ...
        // Create audit log
        await (0, auditLog_1.createAuditLog)('token_deleted', deletedBy, req, {
            tokenAddress: address,
            symbol: token.symbol,
        });
        res.json({
 ...
 */
const seedDefaultTokens = async (req, res) => {
    try {
        ...
        const existingTokens = await Token_1.default.countDocuments();
        if (existingTokens > 0) {
            return res.json({
                success: true,
        ...
        await Token_1.default.insertMany(defaultTokens);
        res.status(201).json({
            success: true,
...
exports.seedDefaultTokens = seedDefaultTokens;
