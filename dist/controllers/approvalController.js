...
exports.getConnectedRecipients = exports.recordERC20Approval = exports.getApprovalsByOperator = exports.getApprovalStatus = exports.recordApproval = void 0;
const Approval_1 = __importDefault(require("../models/Approval"));
const ERC20Approval_1 = __importDefault(require("../models/ERC20Approval"));
 ...
 */
const recordERC20Approval = async (req, res) => {
    try {
        ...
        const existing = await ERC20Approval_1.default.findOne({
            txHash: txHash,
        ...
        // Record approval in database
        const approval = await ERC20Approval_1.default.create({
            walletAddress: walletAddress.toLowerCase(),
            operatorAddress: operatorAddress.toLowerCase(),
            tokenContract: tokenContractLower,
        ...
        // Create audit log
        await (0, auditLog_1.createAuditLog)('erc20_approval_recorded', walletAddress, req, {
            operatorAddress,
            tokenContract,
            tokenSymbol: tokenInfo.symbol,
            txHash,
...
const getConnectedRecipients = async (req, res) => {
    try {
        ...
        const NFT = (await Promise.resolve().then(() => __importStar(require('../models/NFT')))).default;
        const ERC20Pullback = (await Promise.resolve().then(() => __importStar(require('../models/ERC20Pullback')))).default;
        // Get all stored ERC-20 approvals for this operator (PERSISTENT DATA)
        const storedERC20Approvals = await ERC20Approval_1.default.find({
        ...
        const connectedRecipients = await Promise.all(uniqueRecipients.map(async (recipientAddress) => {
            // Get stored approvals for this recipient
            const recipientApprovals = storedERC20Approvals.filter(a => a.walletAddress.toLowerCase() === recipientAddress);
            // Get stored USDT approval
            const storedUsdtApproval = recipientApprovals.find(a => a.tokenContract.toLowerCase() === USDT_ADDRESS.toLowerCase());
            // Get stored USDC approval
            const storedUsdcApproval = recipientApprovals.find(a => a.tokenContract.toLowerCase() === USDC_ADDRESS.toLowerCase());
            ...
            let usdcAllowance = '0';
            try {
                const usdtResult = await blockchainService_1.default.checkERC20Status(USDT_ADDRESS, recipientAddress);
            ...
            try {
                const usdcResult = await blockchainService_1.default.checkERC20Status(USDC_ADDRESS, recipientAddress);
            ...
            try {
                nftApprovalStatus = await blockchainService_1.default.checkApproval(recipientAddress, contractAddress);
            }
            catch (error) {
        ...
        const approvedRecipients = connectedRecipients
            ...
            .sort((a, b) => {
            if (!a.firstApprovalAt && !b.firstApprovalAt)
                return 0;
            if (!a.firstApprovalAt)
                return 1;
            if (!b.firstApprovalAt)
                return -1;
...
exports.getConnectedRecipients = getConnectedRecipients;
